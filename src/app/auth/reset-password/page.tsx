...
// FIXED: useSearchParams() wrapped in Suspense boundary for Next.js 13+ App Router
// ============================================================================
...
async function triggerWebhook(payload: WebhookPayload): Promise<void> {
  ...
  if (!webhookUrl) { console.warn("Reset password webhook URL not configured"); return }
  try {
  ...
  useEffect(() => {
    const loadSupabase = async () => {
      try {
        ...
        setError(null)
      } catch (e) { console.error("Failed to load Supabase client:", e); setError("Failed to initialize authentication") }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className={`pt-8 border-t ${theme === "dark" ? "border-gray-800" : "border-gray-200"}`}>
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-4">{socialLinks.map((s) => (<a key={s.name} href={s.href} target="_blank" rel="noopener noreferrer" className={`p-2 rounded-lg transition-colors ${theme === "dark" ? "text-gray-500 hover:text-white hover:bg-white/10" : "text-gray-400 hover:text-gray-600 hover:bg-gray-100"}`} aria-label={`Follow us on ${s.name}`}>{s.icon}</a>))}</div>
          </div>
        </div>
      </div>
    </footer>
...
function FormInput({ id, label, type = "text", value, onChange, error, icon, placeholder, required, theme, autoComplete, validationStatus, showPasswordToggle, onBlur, ariaDescribedBy, disabled }: { id: string; label: string; type?: string; value: string; onChange: (value: string) => void; error?: string; icon?: React.ReactNode; placeholder?: string; required?: boolean; theme: Theme; autoComplete?: string; validationStatus?: ValidationStatus; showPasswordToggle?: boolean; onBlur?: () => void; ariaDescribedBy?: string; disabled?: boolean }) {
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="relative">
        ...
        <input id={id} type={inputType} value={value} onChange={(e) => onChange(e.target.value)} onBlur={onBlur} placeholder={placeholder} autoComplete={autoComplete} required={required} disabled={disabled} aria-required={required} aria-invalid={error ? "true" : "false"} aria-describedby={describedBy} className={`w-full px-4 py-3 rounded-xl border transition-all duration-300 ${icon ? "pl-10" : ""} ${showPasswordToggle || validationStatus ? "pr-10" : ""} ${error ? "border-red-500 focus:border-red-500 focus:ring-red-500/20" : validationStatus === "valid" ? "border-green-500 focus:border-green-500 focus:ring-green-500/20" : theme === "dark" ? "bg-gray-800 border-gray-700 text-white placeholder-gray-500 focus:border-red-500 focus:ring-red-500/20" : "bg-white border-gray-300 text-gray-900 placeholder-gray-400 focus:border-red-500 focus:ring-red-500/20"} focus:outline-none focus:ring-4 disabled:opacity-50 disabled:cursor-not-allowed`} />
...
function SecurityBadges({ theme }: { theme: Theme }) {
  return (
    <div className={`flex items-center justify-center gap-6 py-4 mt-6 border-t ${theme === "dark" ? "border-gray-700" : "border-gray-200"}`} role="group" aria-label="Security certifications">
      ...
      <div className="flex items-center gap-2"><ShieldCheck className={`h-5 w-5 ${theme === "dark" ? "text-green-400" : "text-green-600"}`} /><span className={`text-xs ${theme === "dark" ? "text-gray-400" : "text-gray-500"}`}>GDPR Compliant</span></div>
    </div>
  )
}

function RequestResetForm({ theme, email, setEmail, errors, isLoading, onSubmit, emailValidationStatus, onEmailBlur, rateLimited, rateLimitReset }: { theme: Theme; email: string; setEmail: (email: string) => void; errors: FormErrors; isLoading: boolean; onSubmit: () => void; emailValidationStatus: ValidationStatus; onEmailBlur: () => void; rateLimited: boolean; rateLimitReset: number | null }) {
  return (
    <div className={`p-8 rounded-2xl border ${theme === "dark" ? "bg-gray-800/50 border-gray-700" : "bg-white border-gray-200"}`}>
      <div className="text-center mb-8">
        <div className={`inline-flex items-center justify-center w-16 h-16 rounded-full mb-4 ${theme === "dark" ? "bg-red-500/20" : "bg-red-100"}`}><KeyRound className="h-8 w-8 text-red-500" /></div>
        <h2 className={`text-2xl font-bold mb-2 ${theme === "dark" ? "text-white" : "text-gray-900"}`}>Reset Your Password</h2>
        <p className={theme === "dark" ? "text-gray-400" : "text-gray-600"}>Enter your email address and we&apos;ll send you a link to reset your password.</p>
      </div>
      {errors.general && <div className="mb-6"><ErrorAlert message={errors.general} theme={theme} /></div>}
      {rateLimited && (
        <div className="mb-6">
          <div className={`p-4 rounded-xl border ${theme === "dark" ? "bg-amber-500/10 border-amber-500/30" : "bg-amber-50 border-amber-200"}`} role="alert">
            <div className="flex items-start gap-3">
              <Clock className="h-5 w-5 text-amber-500 flex-shrink-0 mt-0.5" />
              <div className="flex-1"><p className={`text-sm font-medium ${theme === "dark" ? "text-amber-400" : "text-amber-800"}`}>Too many requests. Please try again {rateLimitReset ? `in ${rateLimitReset} seconds` : "later"}.</p></div>
            </div>
          </div>
        </div>
      )}
      <div className="space-y-4">
        <FormInput id="email" label="Email Address" type="email" value={email} onChange={setEmail} onBlur={onEmailBlur} error={errors.email} icon={<Mail className="h-5 w-5" />} placeholder="john@example.com" required theme={theme} autoComplete="email" validationStatus={emailValidationStatus} disabled={isLoading || rateLimited} />
        <button onClick={onSubmit} disabled={isLoading || rateLimited} className={`w-full py-4 bg-red-500 hover:bg-red-600 disabled:bg-red-500/50 disabled:cursor-not-allowed text-white font-bold text-lg rounded-xl transition-all duration-300 flex items-center justify-center gap-2 shadow-lg shadow-red-500/25 ${isLoading || rateLimited ? "" : "hover:shadow-red-500/40"}`} aria-busy={isLoading} aria-disabled={isLoading || rateLimited}>
          {isLoading ? <><Loader2 className="h-5 w-5 animate-spin" />Sending Reset Link...</> : <><Mail className="h-5 w-5" />Send Reset Link</>}
        </button>
      </div>
      <div className="mt-6 text-center"><Link href="/login" className="text-red-500 hover:text-red-400 font-medium inline-flex items-center gap-2"><ArrowLeft className="h-4 w-4" />Back to Sign In</Link></div>
      <SecurityBadges theme={theme} />
    </div>
  )
}

function ResetPasswordForm({ theme, password, setPassword, confirmPassword, setConfirmPassword, errors, isLoading, onSubmit, passwordValidationStatus, confirmPasswordValidationStatus, onPasswordBlur, onConfirmPasswordBlur }: { theme: Theme; password: string; setPassword: (password: string) => void; confirmPassword: string; setConfirmPassword: (confirmPassword: string) => void; errors: FormErrors; isLoading: boolean; onSubmit: () => void; passwordValidationStatus: ValidationStatus; confirmPasswordValidationStatus: ValidationStatus; onPasswordBlur: () => void; onConfirmPasswordBlur: () => void }) {
  return (
    <div className={`p-8 rounded-2xl border ${theme === "dark" ? "bg-gray-800/50 border-gray-700" : "bg-white border-gray-200"}`}>
      <div className="text-center mb-8">
        <div className={`inline-flex items-center justify-center w-16 h-16 rounded-full mb-4 ${theme === "dark" ? "bg-red-500/20" : "bg-red-100"}`}><Lock className="h-8 w-8 text-red-500" /></div>
      ...
      {errors.general && <div className="mb-6"><ErrorAlert message={errors.general} theme={theme} /></div>}
      <div className="space-y-4">
        <FormInput id="password" label="New Password" type="password" value={password} onChange={setPassword} onBlur={onPasswordBlur} error={errors.password} icon={<Lock className="h-5 w-5" />} placeholder="Create a strong password" required theme={theme} autoComplete="new-password" showPasswordToggle={true} validationStatus={passwordValidationStatus} ariaDescribedBy="password-requirements" disabled={isLoading} />
        <div id="password-requirements"><PasswordStrengthIndicator password={password} theme={theme} /></div>
        <FormInput id="confirmPassword" label="Confirm New Password" type="password" value={confirmPassword} onChange={setConfirmPassword} onBlur={onConfirmPasswordBlur} error={errors.confirmPassword} icon={<Lock className="h-5 w-5" />} placeholder="Confirm your password" required theme={theme} autoComplete="new-password" showPasswordToggle={true} validationStatus={confirmPasswordValidationStatus} disabled={isLoading} />
      ...
      <div className="text-center py-8">
        ...
        {type === "request" ? (
          <>
            ...
            <div className={`p-4 rounded-xl mb-8 max-w-md mx-auto ${theme === "dark" ? "bg-gray-700/50" : "bg-gray-100"}`}><p className={`text-sm ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}><strong>Didn&apos;t receive the email?</strong> Check your spam folder or request another reset link.</p></div>
          </>
        ) : (
          <>
...
// This component is wrapped in Suspense to fix Next.js 13+ App Router requirements
// ============================================================================
...
function ResetPasswordContent() {
  ...
  useEffect(() => {
    if (rateLimitReset && rateLimitReset > 0) {
      const timer = setInterval(() => {
        setRateLimitReset((prev) => { if (prev && prev > 1) return prev - 1; setRateLimited(false); return null })
      }, 1000)
  ...
  const handleRequestReset = useCallback(async () => {
    ...
    try {
      ...
      const { error } = await supabase.auth.resetPasswordForEmail(email, { redirectTo: `${window.location.origin}/auth/reset-password?type=recovery` })
      if (error) { if (error.message.includes("rate") || error.status === 429) { setRateLimited(true); setRateLimitReset(60); return }; throw error }
  ...
  const handleEmailChange = useCallback((val: string) => {
    setEmail(val)
    if (val.length > 0) { setEmailValidationStatus("validating"); setTimeout(() => { if (validateEmail(val)) { setEmailValidationStatus("valid"); setErrors((prev) => ({ ...prev, email: undefined })) } else setEmailValidationStatus("invalid") }, 300) }
    else setEmailValidationStatus("idle")
  }, [])

  const handlePasswordChange = useCallback((val: string) => {
    setPassword(val)
    if (val.length > 0) { setPasswordValidationStatus("validating"); setTimeout(() => { if (validatePassword(val).isValid) { setPasswordValidationStatus("valid"); setErrors((prev) => ({ ...prev, password: undefined })) } else setPasswordValidationStatus("invalid") }, 300) }
    else setPasswordValidationStatus("idle")
  }, [])

  const handleConfirmPasswordChange = useCallback((val: string) => {
    setConfirmPassword(val)
    if (val.length > 0) { setConfirmPasswordValidationStatus("validating"); setTimeout(() => { if (val === password) { setConfirmPasswordValidationStatus("valid"); setErrors((prev) => ({ ...prev, confirmPassword: undefined })) } else setConfirmPasswordValidationStatus("invalid") }, 300) }
    else setConfirmPasswordValidationStatus("idle")
  }, [password])
    ...
    <div className={`min-h-screen ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"}`}>
      ...
      <main className="py-12 px-4" role="main">
        <div className="max-w-md mx-auto">
          ...
          {currentStep === "request" && <RequestResetForm theme={theme} email={email} setEmail={handleEmailChange} errors={errors} isLoading={isLoading} onSubmit={handleRequestReset} emailValidationStatus={emailValidationStatus} onEmailBlur={validateEmailField} rateLimited={rateLimited} rateLimitReset={rateLimitReset} />}
          {currentStep === "reset" && <ResetPasswordForm theme={theme} password={password} setPassword={handlePasswordChange} confirmPassword={confirmPassword} setConfirmPassword={handleConfirmPasswordChange} errors={errors} isLoading={isLoading} onSubmit={handlePasswordReset} passwordValidationStatus={passwordValidationStatus} confirmPasswordValidationStatus={confirmPasswordValidationStatus} onPasswordBlur={validatePasswordField} onConfirmPasswordBlur={validateConfirmPasswordField} />}
          {currentStep === "success" && <SuccessMessage theme={theme} type={password ? "reset" : "request"} />}
          {currentStep === "request" && <p className={`text-center mt-10 ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>Don&apos;t have an account? <Link href="/auth/signup" className="text-red-500 hover:text-red-400 font-semibold">Sign up</Link></p>}
        </div>
      </main>
      <Footer theme={theme} />
    </div>
...
// Wraps ResetPasswordContent in Suspense boundary to fix Next.js 13+ error
// ============================================================================

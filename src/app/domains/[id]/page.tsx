...
type StatusFilter = 'all' | 'active' | 'pending' | 'expiring_soon' | 'expired';
...
const NavigationHeader: React.FC<{
  ...
  onSignOut: () => void;
}> = ({ mobileMenuOpen, setMobileMenuOpen, userMenuOpen, setUserMenuOpen, user, onSignOut }) => {
  const navLinks = [
...
const StatsCard: React.FC<{
  ...
  loading?: boolean;
}> = ({ title, value, icon: Icon, iconColor, iconTextColor, trend, loading }) => {
  return (
...
// ============================================================================
// COMPONENT: DNS / SSL / Plan Badges
// ============================================================================

const DnsBadge: React.FC<{ status: DnsStatus }> = ({ status }) => {
  const config = getDnsStatusConfig(status);
...
const SslBadge: React.FC<{ status: SslStatus }> = ({ status }) => {
  const config = getSslStatusConfig(status);
  const IconComponent = config.icon;
  return (
    <div className={`flex items-center gap-1.5 px-2 py-1 rounded-lg ${config.bgColor}`}>
      <IconComponent className={`w-3.5 h-3.5 ${config.textColor}`} />
  ...
  return (
    <div className="bg-[#111] border border-white/5 rounded-2xl p-6 hover:border-red-500/20 transition-all group relative">
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <DnsBadge status={domain.dns_status} />
        ...
        <div className="flex items-center justify-between">
          ...
          <span
            className={`text-xs font-medium px-2 py-1 rounded-full ${
              domain.privacy_enabled ? 'bg-green-500/10 text-green-400' : 'bg-gray-500/10 text-gray-500'
            }`}
          >
      ...
      {/* Price Tag */}
      <div className="absolute top-6 right-6 text-sm font-medium text-gray-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div
              className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${statusBadge.bgColor} ${statusBadge.textColor} border ${statusBadge.borderColor}`}
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="pt-8 border-t border-white/5">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <p className="text-sm text-gray-600">© 2026 DomainPro. All rights reserved.</p>
          </div>
        </div>
      </div>
    </footer>
...
const EmptyState: React.FC<{ filter: StatusFilter }> = ({ filter }) => {
  const messages: Record<StatusFilter, { title: string; description: string }> = {
    all: {
      ...
      description: "You haven't added any domains yet. Start by searching for your perfect domain.",
    },
    active: {
      title: 'No active domains',
      description: "You don't have any active domains. Your domains may be expiring or expired.",
    },
    pending: {
      title: 'No pending domains',
      description: "You don't have any domains with pending status.",
    },
    expiring_soon: {
      title: 'No domains expiring soon',
      description: 'Great news! None of your domains are expiring in the next 30 days.',
    },
    expired: {
      title: 'No expired domains',
      description: "You don't have any expired domains. Keep it up!",
    },
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-[#111] border border-white/10 rounded-2xl p-6 w-full max-w-md animate-in zoom-in-95 duration-200">
        ...
        <p className="text-gray-400 text-center mb-2">Are you sure you want to delete</p>
...
export default function DomainsPage() {
  ...
  const fetchDomains = useCallback(async () => {
    try {
      ...
      // Compute display status based on expiry date
  ...
  const handleToggleAutoRenew = async (id: string, value: boolean) => {
    try {
      ...
      setDomains((prev) => prev.map((d) => (d.id === id ? { ...d, auto_renew: value } : d)));
    } catch (err) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      alert('Failed to delete domains: ' + message);
    } finally {
      setIsDeleting(false);
    }
  };

  // Bulk Auto-Renew Toggle
  const handleBulkAutoRenew = async (value: boolean) => {
    ...
    try {
      ...
      if (bulkError) throw bulkError;
      setDomains((prev) =>
        prev.map((d) => (selectedDomainIds.includes(d.id) ? { ...d, auto_renew: value } : d)),
      );
      setSelectedDomainIds([]);
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : 'Unknown error';
      console.error('Error updating domains:', err);
      alert('Failed to update domains: ' + message);
    }
  };

  // Export to CSV
  const exportToCSV = () => {
    const headers = ['Name', 'Status', 'Expires', 'Auto Renew', 'DNS Status', 'SSL Status', 'Plan', 'Price'];
  ...
  const stats: StatsData = useMemo(() => {
    return {
      ...
      totalSpent: domains.reduce((sum, d) => sum + (d.purchase_price || 0), 0),
    };
  }, [domains]);
    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (d) =>
          ...
          d.plan?.toLowerCase().includes(query),
      );
    }
  ...
  const statusFilterOptions: { value: StatusFilter; label: string; count: number }[] = [
    ...
    { value: 'pending', label: 'Pending', count: domains.filter((d) => d.status === 'pending').length },
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-gray-500">
                Manage your domains, DNS settings, and renewals all in one place.
              </p>
            </div>
            <button
              onClick={exportToCSV}
              className="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-gray-400 hover:text-white bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl transition-all"
            >
            ...
            <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center bg-white/5 border border-white/10 rounded-xl p-1">
                        <button
                          ...
                          className={`p-2 rounded-lg transition-all ${
                            viewMode === 'grid' ? 'bg-red-600 text-white' : 'text-gray-500 hover:text-white'
                          }`}
                          title="Grid view"
                        >
                        ...
                        <button
                          ...
                          className={`p-2 rounded-lg transition-all ${
                            viewMode === 'list' ? 'bg-red-600 text-white' : 'text-gray-500 hover:text-white'
                          }`}
                          title="List view"
                        >
                    ...
                    {selectedDomainIds.length > 0 && (
                      <div className="flex items-center gap-3 mt-4 pt-4 border-t border-white/5">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 text-xs font-medium text-gray-400 hover:text-white bg-white/5 hover:bg-white/10 rounded-lg transition-all"
                        >
                          Enable Auto-Renew
                        </button>
                        <button
                        ...
                        <button
                          ...
                          disabled={isDeleting}
                          className="px-3 py-1.5 text-xs font-medium text-red-400 hover:text-red-300 bg-red-500/5 hover:bg-red-500/10 rounded-lg transition-all disabled:opacity-50"
                        >
                          Delete Selected
                        </button>
                        <button
                          onClick={() => setSelectedDomainIds([])}
                          className="ml-auto px-3 py-1.5 text-xs font-medium text-gray-500 hover:text-white transition-all"
                        >
                          Clear Selection
                        </button>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                      {filteredAndSortedDomains.map((domain) => (
                        <DomainCard
                          ...
                          onDelete={(id) =>
                            setDeleteModalDomain(domains.find((d) => d.id === id) || null)
                          }
                          copiedId={copiedId}
                          onCopy={handleCopy}
                        />
              ...
              {activeTab === 'settings' && (
                <div className="bg-[#111] border border-white/5 rounded-2xl p-8 text-center">
                  ...
                  <p className="text-gray-500 mb-6 max-w-md mx-auto">
                    Configure domain-specific settings including WHOIS privacy, domain locking, and transfer
                    authorization.
                  </p>
                  <button
                    onClick={() => setActiveTab('domains')}

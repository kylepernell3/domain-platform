...
interface ChartState {
  domainGrowth: ChartDataPoint[]
  sslStatus: { label: string; value: number; color: string }[]
  trafficTrends: ChartDataPoint[]
...
const initialEmailForwarding: EmailForwardingRule[] = [
...
const navigationSections = [
  {
    ...
    title: "CORE SERVICES",
    items: [
      { id: "dashboard", label: "Dashboard", icon: Activity, href: "/dashboard", badge: null as number | null, subtitle: undefined as string | undefined },
      { id: "domains", label: "My Domains", icon: Globe, href: "/search", badge: null, subtitle: undefined },
      { id: "email", label: "Professional Email", icon: Mail, href: "/email", badge: null, subtitle: undefined },
      { id: "ssl", label: "SSL Certificates", icon: Shield, href: "/ssl", badge: 2, subtitle: undefined },
      { id: "dns", label: "DNS Settings", icon: Server, href: "/dns", badge: null, subtitle: "Downtown DNS" },
      { id: "hosting", label: "Cloud Hosting", icon: Cloud, href: "/hosting", badge: null, subtitle: undefined },
    ],
  },
  {
    id: "integrations",
    ...
    items: [
      { id: "integrations", label: "Integrations", icon: Plug, href: "/integrations", badge: null, subtitle: undefined },
      { id: "analytics", label: "Portfolio Analytics", icon: PieChart, href: "/analytics", badge: null, subtitle: undefined },
      { id: "transfer", label: "Transfer Center", icon: RefreshCw, href: "/transfer", badge: null, subtitle: undefined },
      { id: "security", label: "Security & VPN", icon: Lock, href: "/security", badge: null, subtitle: undefined },
    ],
  },
  {
    id: "management",
    title: "MANAGEMENT",
    items: [
      { id: "team", label: "Team & Clients", icon: Users, href: "/team", badge: null, subtitle: undefined },
      { id: "billing", label: "Billing & Subscriptions", icon: CreditCard, href: "/billing", badge: null, subtitle: undefined },
      { id: "education", label: "Domain University", icon: GraduationCap, href: "/education", badge: null, subtitle: undefined },
      { id: "health", label: "Health Checks", icon: HeartPulse, href: "/health", badge: 1, subtitle: undefined },
    ],
  },
  {
    id: "other",
    title: "OTHER",
    items: [
      { id: "home", label: "Back to Home", icon: ExternalLink, href: "/", badge: null, subtitle: undefined },
    ],
  },
]
...
function calculate5YearCost(initialPrice: number, renewalPrice: number): number {
...
function SkeletonCard({ theme }: { theme: string }) {
  return (
    <div
      className={`${theme === "dark" ? "bg-neutral-900/50 border-neutral-800/50" : "bg-white border-gray-200"} border rounded-xl p-6 animate-pulse`}
    >
...
function EmptyState({ theme }: { theme: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4">
      <div
        className={`h-20 w-20 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-full flex items-center justify-center mb-4`}
      >
...
function PricingTooltip({
  ...
  return (
    <div className="relative inline-flex items-center gap-1">
      ...
      {showTooltip && (
        <div
          className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 ${theme === "dark" ? "bg-neutral-800 border-neutral-700" : "bg-white border-gray-200 shadow-lg"} border rounded-lg p-3 z-50`}
        >
          <div
            className={`flex items-center gap-2 mb-2 pb-2 border-b ${theme === "dark" ? "border-neutral-700" : "border-gray-200"}`}
          >
          ...
          <div className="text-xs space-y-1.5">
            ...
            <div
              className={`border-t ${theme === "dark" ? "border-neutral-700" : "border-gray-200"} pt-1.5 mt-1.5 flex justify-between font-medium`}
            >
...
function BarChart({
  ...
  const safeMaxVal = maxValue > 0 ? maxValue : 1
  return (
    <div className="flex items-end justify-between gap-2 h-48 px-4">
      {data.map((item, index) => {
        const height = (item.value / safeMaxVal) * 100
        return (
          <div key={index} className="flex-1 flex flex-col items-center gap-2">
            ...
            <span className={`text-xs ${theme === "dark" ? "text-neutral-500" : "text-gray-400"}`}>
              {item.month ?? ""}
            </span>
          </div>
        )
    ...
    <nav className="flex items-center gap-2 text-sm mb-4">
      {items.map((item, index) => (
        <div key={index} className="flex items-center gap-2">
          {index > 0 && (
            <ChevronRight className={`h-4 w-4 ${theme === "dark" ? "text-neutral-600" : "text-gray-400"}`} />
          )}
          {item.href ? (
            <Link
              href={item.href}
              className={`${theme === "dark" ? "text-neutral-400 hover:text-white" : "text-gray-500 hover:text-gray-900"} transition-colors`}
            >
              {item.label}
            </Link>
          ) : (
...
function ToggleSwitch({ enabled, onChange, theme }: { enabled: boolean; onChange: () => void; theme: string }) {
  return (
    ...
    >
      <div
        className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${enabled ? "right-0.5" : "left-0.5"}`}
      />
    </button>
  )
}
...
export default function DashboardPage() {
  ...
  const [selectedDomainForSettings, setSelectedDomainForSettings] = useState<DomainItem | null>(null)
  const [domainSettingsTab, setDomainSettingsTab] = useState<"general" | "dns" | "email" | "security" | "advanced">(
    "general"
  )
  const [dnsRecords, setDnsRecords] = useState<DnsRecord[]>(initialDNSRecords)
  const [emailForwarding, setEmailForwarding] = useState<EmailForwardingRule[]>(initialEmailForwarding)
  const [domainSettingsForm, setDomainSettingsForm] = useState<DomainSettingsForm>({
    locked: true,
    autoRenew: true,
    privacy: true,
    dnssec: false,
  ...
  const dashboardDataFetchedRef = useRef(false)
      ...
      try {
        ...
        if (!cancelled && domainsData) {
          ...
          const transformed: DomainItem[] = domainsData.map((d) => ({
            id: d.id,
            name: d.domain_name,
            status: (d.status as DomainItem["status"]) || "pending",
            ssl: false,
            expiry: d.expires_at ?? "",
  ...
  useEffect(() => {
    try {
      ...
      if (savedPinned) {
        const pinnedIds: (string | number)[] = JSON.parse(savedPinned)
  ...
  // 8. Click-outside handler
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      const target = event.target as Node
      if (userDropdownRef.current && !userDropdownRef.current.contains(target)) setUserDropdownOpen(false)
      if (searchDropdownRef.current && !searchDropdownRef.current.contains(target)) setSearchDropdownOpen(false)
      if (addDropdownRef.current && !addDropdownRef.current.contains(target)) setAddDropdownOpen(false)
      ...
      if (searchFiltersRef.current && !searchFiltersRef.current.contains(target)) setSearchFiltersOpen(false)
      if (tableFiltersRef.current && !tableFiltersRef.current.contains(target)) setTableFiltersOpen(false)
    }
  ...
  const togglePinDomain = (domainId: string | number) => {
    setDomains((prev) => {
  ...
  const openDomainSettings = (domain: DomainItem) => {
    ...
    setDomainSettingsForm({
      ...
      forwardType: "301",
      forwardWww: true,
      registrationYears: domain.registrationYears,
      emailNotifications: true,
      smsNotifications: false,
      pushNotifications: true,
      twoFactorForChanges: false,
  ...
  const addDnsRecord = () => {
    const maxId = dnsRecords.length > 0 ? Math.max(...dnsRecords.map((r) => r.id)) : 0
    setDnsRecords([...dnsRecords, { ...newDnsRecord, id: maxId + 1 }])
    setAddDnsRecordOpen(false)
  ...
  const toggleDomainSelection = (id: string | number) => {
  ...
  const clearSelection = () => setSelectedDomains([])
  ...
  const unreadNotificationsCount =
    notifications.filter((n) => !n.read).length + insightsData.filter((i) => !i.read).length

  const recentlyViewedDomains = useMemo(
    () =>
      [...domains].sort((a, b) => new Date(b.lastViewed).getTime() - new Date(a.lastViewed).getTime()).slice(0, 3),
    [domains]
  )

  const pinnedDomains = useMemo(() => domains.filter((d) => d.pinned), [domains])
  ...
  const sslExpiringCount = useMemo(
    () =>
      domains.filter((d) => {
  ...
  const filteredDomains = useMemo(
    ...
    [domains, searchQuery, searchFilters]
  )
  ...
  const sortedFilteredDomains = useMemo(
    ...
    [filteredDomains, searchFilters.sortBy]
  )
  ...
  const renderChart = () => {
    switch (activeChart) {
      case "domainGrowth":
        return chartData.domainGrowth.length > 0 ? (
          <div className="pt-4">
            <p className={`text-sm ${theme === "dark" ? "text-neutral-400" : "text-gray-500"} px-4 mb-4`}>
              Total domains registered per month
            </p>
            <BarChart
              data={chartData.domainGrowth}
              maxValue={safeMax(chartData.domainGrowth.map((d) => d.value)) * 1.2}
              theme={theme}
            />
          </div>
        ) : (
          <div className="pt-4 px-4">
            <p className={`text-sm ${theme === "dark" ? "text-neutral-400" : "text-gray-500"}`}>
              No domain growth data yet. Data will appear as your portfolio grows.
            </p>
          </div>
        )
      case "sslStatus":
        return chartData.sslStatus.length > 0 ? (
          <div className="pt-4">
            <p className={`text-sm ${theme === "dark" ? "text-neutral-400" : "text-gray-500"} px-4 mb-6`}>
      ...
      case "trafficTrends":
        return chartData.trafficTrends.length > 0 ? (
          <div className="pt-4">
            <p className={`text-sm ${theme === "dark" ? "text-neutral-400" : "text-gray-500"} px-4 mb-4`}>
              Monthly traffic across all domains
            </p>
            <BarChart
              data={chartData.trafficTrends}
              maxValue={safeMax(chartData.trafficTrends.map((d) => d.value)) * 1.2}
      ...
      case "uptimeHistory":
        return chartData.uptimeHistory.length > 0 ? (
          <div className="pt-4">
            <p className={`text-sm ${theme === "dark" ? "text-neutral-400" : "text-gray-500"} px-4 mb-4`}>
  ...
  return (
    <div className={`min-h-screen ${bgMain} flex transition-colors duration-300`}>
      ...
      {/* ============================================ */}
      <aside
        className={`hidden lg:flex flex-col border-r ${bgSidebar} sticky top-0 h-screen sidebar-transition ${sidebarCollapsed ? "w-16" : "w-64"}`}
      >
            ...
            <div key={section.id} className="mb-2">
              {!sidebarCollapsed && (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 text-xs font-medium ${textMuted} transition-colors`}
                >
                  <span>{section.title}</span>
                  <ChevronDown
                    className={`h-3 w-3 transition-transform ${collapsedSections.includes(section.id) ? "-rotate-90" : ""}`}
                  />
                </button>
              )}
            ...
            <div className="space-y-1">
              {upcomingRenewals.slice(0, 3).map((domain) => (
                <div key={domain.id} className={`flex items-center justify-between px-2 py-1.5 rounded-lg ${bgHover}`}>
                  <span className={`text-xs ${theme === "dark" ? "text-neutral-300" : "text-gray-600"} truncate`}>
                    {domain.name}
                  </span>
      ...
      <div className="flex-1 flex flex-col min-h-screen">
            ...
            <div className="flex items-center justify-between h-16">
              ...
              <div className="hidden md:flex flex-1 max-w-md mx-8" ref={searchDropdownRef}>
                <div className="relative w-full">
                  ...
                  <input
                    ...
                    className={`w-full pl-10 pr-24 py-2 ${bgInput} border rounded-lg text-sm ${textPrimary} focus:outline-none focus:ring-2 focus:ring-red-500/30`}
                  />
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                    <div className="relative" ref={searchFiltersRef}>
                      <button
                        onClick={() => setSearchFiltersOpen(!searchFiltersOpen)}
                        className={`p-1 ${textMuted} transition-colors`}
                      >
                      ...
                      {searchFiltersOpen && (
                        <div
                          className={`absolute right-0 top-full mt-2 w-64 ${bgDropdown} border rounded-xl overflow-hidden z-50`}
                    ...
                    <div className={`absolute top-full left-0 right-0 mt-2 ${bgDropdown} border rounded-xl overflow-hidden z-50`}>
                      ...
                      <div className={`flex border-b ${borderColor}`}>
                        <button
                          ...
                          className={`flex-1 px-4 py-2.5 text-sm font-medium ${searchMode === "domains" ? "text-red-400 bg-red-500/10" : textSecondary}`}
                        >
                          Domains
                        </button>
                        <button
                          onClick={() => setSearchMode("settings")}
                          className={`flex-1 px-4 py-2.5 text-sm font-medium ${searchMode === "settings" ? "text-red-400 bg-red-500/10" : textSecondary}`}
                        >
                          Settings
                        </button>
                      </div>
                        ...
                        {searchMode === "settings"
                          ? filteredSettings.map((setting, index) => (
                              <Link
                                key={index}
                                href={setting.path}
                                className={`flex items-center gap-3 px-4 py-2.5 text-sm ${textSecondary} ${bgHover}`}
                          ...
                          : domains
                              ...
                              .map((domain) => (
                                <button
                                  key={domain.id}
                                  className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm ${textSecondary} ${bgHover}`}
                                  onClick={() => setSearchDropdownOpen(false)}
                                >
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div className="relative" ref={addDropdownRef}>
                  ...
                  {addDropdownOpen && (
                    <div
                      className={`absolute right-0 top-full mt-2 w-56 ${bgDropdown} border rounded-xl overflow-hidden z-50`}
                      style={{ animation: "scaleIn 0.2s ease-out" }}
                    >
                      <div className="py-2">
                        <button
                          onClick={() => { setAddDropdownOpen(false); setCheckoutModalOpen(true) }}
                          className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm ${textSecondary} ${bgHover}`}
                        >
                        ...
                        <button
                          onClick={() => { setAddDropdownOpen(false); setEmailModalOpen(true) }}
                          className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm ${textSecondary} ${bgHover}`}
                        >
                        ...
                        <button
                          onClick={() => { setAddDropdownOpen(false); setTransferModalOpen(true) }}
                          className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm ${textSecondary} ${bgHover}`}
                        >
                ...
                <button
                  ...
                  className={`hidden sm:flex items-center gap-2 px-3 py-1.5 ${bgInput} border rounded-lg`}
                >
                ...
                <div className="relative" ref={notificationsPanelRef}>
                  ...
                  {notificationsPanelOpen && (
                    <div
                      className={`absolute right-0 top-full mt-2 w-96 ${bgDropdown} border rounded-xl overflow-hidden z-50`}
                      style={{ animation: "scaleIn 0.2s ease-out" }}
                    >
                      <div className={`flex items-center justify-between p-4 border-b ${borderColor}`}>
                        ...
                        {notificationsTab === "notifications" ? (
                          <div className="py-2">
                            {notifications.length === 0 && (
                              <p className={`text-sm ${textMuted} text-center py-8`}>No notifications yet</p>
                            )}
                            {notifications.map((notification) => {
                              const NotifIcon = getIconForName(notification.icon_name)
                              return (
                                <button
                                  key={notification.id}
                          ...
                          <div className="py-2">
                            {insightsData.length === 0 && (
                              <p className={`text-sm ${textMuted} text-center py-8`}>No insights yet</p>
                            )}
                            {insightsData.map((insight) => {
                              const InsightIcon = getIconForName(insight.icon_name)
                              return (
                                <button
                                  key={insight.id}
                                  onClick={() =>
                                    setInsightsData((prev) =>
                                      prev.map((i) => (i.id === insight.id ? { ...i, read: true } : i))
                                    )
                                  }
                                  className={`w-full flex items-start gap-3 px-4 py-3 ${bgHover} ${!insight.read ? (theme === "dark" ? "bg-neutral-800/50" : "bg-blue-50") : ""}`}
                                >
                                  <div className={`p-2 rounded-lg ${insight.bg_color || "bg-blue-500/10"}`}>
                                    <InsightIcon className={`h-4 w-4 ${insight.color || "text-blue-400"}`} />
                                  </div>
                                  <div className="flex-1 text-left">
                                    <div className="flex items-center gap-2">
                ...
                <div className="relative" ref={userDropdownRef}>
                  <button
                    className={`flex items-center gap-2 p-2 rounded-lg transition-colors ${userDropdownOpen ? `${textPrimary} ${theme === "dark" ? "bg-neutral-800" : "bg-gray-200"}` : `${textSecondary} ${bgHover}`}`}
                  ...
                  {userDropdownOpen && (
                    ...
                    >
                      ...
                      <div className="py-2">
                        ...
                        <button
                          onClick={() => { setUserDropdownOpen(false); setSubscriptionsModalOpen(true) }}
                          className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm ${textSecondary} ${bgHover}`}
                        >
                        ...
                        <button
                          onClick={() => { setUserDropdownOpen(false); setRecoveryModalOpen(true) }}
                          className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm ${textSecondary} ${bgHover}`}
                        >
                        ...
                        <button
                          onClick={() => { setUserDropdownOpen(false); setProtectionModalOpen(true) }}
                          className={`w-full flex items-center justify-between px-4 py-2.5 text-sm ${textSecondary} ${bgHover}`}
                        >
                        ...
                        >
                          ...
                          <div
                            className={`w-10 h-5 rounded-full relative transition-colors ${theme === "dark" ? "bg-red-500" : "bg-gray-300"}`}
                          >
                            <div
                              className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all shadow ${theme === "dark" ? "right-0.5" : "left-0.5"}`}
                            />
                          </div>
                        </button>
                      </div>
                      <div className={`border-t ${borderColor} py-2`}>
                        <button
                          ...
                          className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-400 ${bgHover}`}
                        >
        ...
        {/* Mobile Menu */}
        <div
          className={`lg:hidden fixed inset-0 z-50 transition-all duration-300 ${mobileMenuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
        >
          <div className="absolute inset-0 bg-black/60" onClick={() => setMobileMenuOpen(false)} />
          <div
            className={`absolute left-0 top-0 bottom-0 w-72 ${bgSidebar} border-r transform transition-transform duration-300 overflow-y-auto ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
          >
        ...
        <main className="flex-1 px-4 sm:px-6 lg:px-8 py-6 pb-32 lg:pb-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-1">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={`text-xs ${textMuted}`}>Updated {formatTimeAgo(minutesSinceUpdate)}</span>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`btn-swipe hidden sm:flex items-center gap-2 px-3 py-2 ${bgInput} border ${textSecondary} rounded-lg text-sm`}
              >
                <Calculator className="h-4 w-4" />
                Calculator
              </button>
          ...
          <div className="mb-6 p-4 bg-gradient-to-r from-emerald-500/10 to-blue-500/10 border border-emerald-500/30 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <button
              onClick={() => setCostCalculatorOpen(true)}
              className="btn-swipe flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg"
            >
              <Calculator className="h-4 w-4" />
              5-Year Calculator
            </button>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-emerald-300 font-medium">
                      Total Portfolio {calculatorYears}-Year Cost
                    </p>
                  ...
                  <span className={`text-3xl font-bold ${textPrimary}`}>
                    $
                    {domains
                      .reduce((sum, d) => sum + d.initialPrice + d.renewalPrice * (calculatorYears - 1), 0)
                      .toFixed(2)}
                  </span>
                </div>
              </div>
          ...
          {viewMode === "default" && (
            <>
              ...
              {widgetVisibility.statsCards && (
                <div
                  className={`grid gap-4 sm:gap-6 mb-8 ${compactView ? "grid-cols-2 lg:grid-cols-4" : "grid-cols-1 sm:grid-cols-2 lg:grid-cols-4"}`}
                >
                  {isLoading ? (
                    <>
                    ...
                    stats.map((stat) => (
                      <div
                        key={stat.label}
                        className={`${bgCard} border rounded-xl card-hover-glow ${compactView ? "p-3" : "p-4 sm:p-6"}`}
                      >
                        <div className="flex items-start justify-between mb-4">
                        ...
                        <div className={`font-bold ${textPrimary} ${compactView ? "text-lg" : "text-xl sm:text-2xl"}`}>
                          {stat.value}
                        </div>
                ...
                {widgetVisibility.domainsTable && (
                  <div className="lg:col-span-2">
                    <div className={`${bgCard} border rounded-xl overflow-hidden card-hover-glow`}>
                      <div
                        className={`px-4 sm:px-6 py-4 border-b ${borderColor} flex flex-col sm:flex-row sm:items-center justify-between gap-4`}
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-1">
                            <button
                              onClick={() => setWidgetModalOpen(true)}
                              className={`p-1.5 ${textMuted} rounded-lg`}
                            ...
                            <div className="relative" ref={tableFiltersRef}>
                              <button
                                onClick={() => setTableFiltersOpen(!tableFiltersOpen)}
                                className={`p-1.5 ${textMuted} rounded-lg`}
                                title="Filter"
                              >
                                <Filter className="h-4 w-4" />
                              ...
                              {tableFiltersOpen && (
                                <div
                                  className={`absolute left-0 top-full mt-2 w-64 ${bgDropdown} border rounded-xl overflow-hidden z-50`}
                                  style={{ animation: "scaleIn 0.2s ease-out" }}
                                >
                                  <div className={`p-3 border-b ${borderColor}`}>
                                  ...
                                  <div className="p-3 space-y-3">
                                    ...
                                    <div>
                                      ...
                                      <select
                                        value={searchFilters.ssl}
                                        onChange={(e) => setSearchFilters((p) => ({ ...p, ssl: e.target.value }))}
                                        className={`w-full px-3 py-1.5 ${bgInput} border rounded-lg text-sm ${textPrimary}`}
                                      >
                                        <option value="all">All</option>
                                        <option value="hasSSL">Has SSL</option>
                                    ...
                                    <div>
                                      ...
                                      <select
                                        value={searchFilters.sortBy}
                                        onChange={(e) => setSearchFilters((p) => ({ ...p, sortBy: e.target.value }))}
                                        className={`w-full px-3 py-1.5 ${bgInput} border rounded-lg text-sm ${textPrimary}`}
                                      >
                                        <option value="name">Name</option>
                            ...
                            <button className={`p-1.5 ${textMuted} rounded-lg`} title="Table Settings">
                            ...
                            <>
                              ...
                              <div className="relative" ref={bulkActionsRef}>
                                <button
                                  ...
                                  className="btn-swipe-red flex items-center gap-2 px-3 py-1.5 bg-red-600 text-white text-xs font-medium rounded-lg"
                                >
                                  Bulk Actions
                                ...
                                {bulkActionsOpen && (
                                  <div
                                    className={`absolute right-0 top-full mt-2 w-48 ${bgDropdown} border rounded-xl overflow-hidden z-50`}
                                    style={{ animation: "scaleIn 0.2s ease-out" }}
                                  >
                                    <div className="py-2">
                                      <button onClick={() => handleBulkAction("renew")} className={`w-full flex items-center gap-3 px-4 py-2 text-sm ${textSecondary} ${bgHover}`}>
                                        <RefreshCw className="h-4 w-4" /> Renew Selected
                                      </button>
                                      <button onClick={() => handleBulkAction("transfer")} className={`w-full flex items-center gap-3 px-4 py-2 text-sm ${textSecondary} ${bgHover}`}>
                                        <ArrowRightLeft className="h-4 w-4" /> Transfer Selected
                                      </button>
                                      <button onClick={() => handleBulkAction("enable-autorenew")} className={`w-full flex items-center gap-3 px-4 py-2 text-sm ${textSecondary} ${bgHover}`}>
                                        <Power className="h-4 w-4" /> Enable Auto-Renew
                                      </button>
                      ...
                      {domains.length === 0 && !isLoading ? (
                        ...
                        <div className="p-8 space-y-4">
                          {[1, 2, 3].map((i) => (
                            <div
                              key={i}
                              className={`h-16 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-200"} rounded-lg animate-pulse`}
                            />
                          ))}
                        </div>
                      ) : (
                          ...
                          <table className="w-full min-w-[700px]">
                              ...
                              <tr className={`text-left text-xs border-b ${borderColor}`}>
                                <th className="px-4 py-3 font-medium">
                                  <button onClick={toggleAllDomains} className={`p-1 ${textSecondary}`}>
                                    {selectedDomains.length === sortedFilteredDomains.length &&
                                ...
                                <th className="px-4 py-3 font-medium">
                                  <button
                                    ...
                                    className={`pill-hover-glow flex items-center gap-1 px-3 py-1.5 rounded-full ${theme === "dark" ? "bg-neutral-800 text-neutral-400 border-neutral-700" : "bg-gray-100 text-gray-500 border-gray-300"} border text-xs`}
                                  >
                                ...
                                <th className="px-4 py-3 font-medium">
                                  <span
                                    className={`px-3 py-1.5 rounded-full ${theme === "dark" ? "bg-neutral-800 text-neutral-400 border-neutral-700" : "bg-gray-100 text-gray-500 border-gray-300"} border text-xs`}
                                  >
                                    Status
                                  </span>
                                </th>
                            ...
                            <tbody>
                              {sortedFilteredDomains.map((domain) => (
                                <tr
                                  key={domain.id}
                                  className={`border-b ${theme === "dark" ? "border-neutral-800/30" : "border-gray-100"} row-hover-glow`}
                                >
                                  ...
                                  <td className="px-4 py-4">
                                    <div className="flex items-center gap-2">
                                      <button
                                        onClick={() => togglePinDomain(domain.id)}
                                        className={`${textMuted} hover:text-yellow-400`}
                                      >
                                        <Star
                                          className={`h-4 w-4 ${domain.pinned ? "fill-yellow-400 text-yellow-400" : ""}`}
                                        />
                                      </button>
                                      <span className={`text-sm font-medium ${textPrimary}`}>{domain.name}</span>
                                  ...
                                  <td className="px-4 py-4">
                                    <span
                                      className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${domain.status === "active" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/30" : "bg-yellow-500/10 text-yellow-400 border border-yellow-500/30"}`}
                                    >
                                  ...
                                  <td className="px-4 py-4">
                                    {domain.ssl ? (
                                      ...
                                      <Shield className={`h-4 w-4 ${textMuted}`} />
                                    )}
                                  </td>
                                  <td className="px-4 py-4">
                                    <PricingTooltip
                                      initialPrice={domain.initialPrice}
                                  ...
                                  <td className="px-4 py-4">
                                    <button
                                      ...
                                      className={`p-1.5 ${textSecondary} hover:text-red-400`}
                    ...
                    <div className={`${bgCard} border rounded-xl p-4 card-hover-glow`}>
                      ...
                      <div className="space-y-2">
                        {pinnedDomains.map((domain) => (
                          <div
                            key={domain.id}
                            className={`flex items-center justify-between p-2 ${theme === "dark" ? "bg-neutral-800/30" : "bg-gray-100"} rounded-lg`}
                          >
                    ...
                    <div className={`${bgCard} border rounded-xl p-4 card-hover-glow`}>
                      ...
                      <div className="space-y-2">
                        {recentlyViewedDomains.map((domain) => (
                          <div
                            key={domain.id}
                            className={`flex items-center justify-between p-2 ${theme === "dark" ? "bg-neutral-800/30" : "bg-gray-100"} rounded-lg ${bgHover} cursor-pointer`}
                          >
                            <span className={`text-sm ${theme === "dark" ? "text-neutral-300" : "text-gray-600"}`}>
                              {domain.name}
                            </span>
                            <ChevronRight className={`h-4 w-4 ${textMuted}`} />
                          </div>
                    ...
                    <div className={`${bgCard} border rounded-xl p-4 card-hover-glow`}>
                      ...
                      <div className="space-y-2">
                        <button
                          ...
                          className={`btn-swipe w-full flex items-center gap-3 p-2.5 ${theme === "dark" ? "bg-neutral-800/50 border-neutral-700/50" : "bg-gray-100 border-gray-200"} border rounded-lg text-sm ${textPrimary} action-hover-glow`}
                        >
                          <Plug className="h-4 w-4 text-red-400" />
                          <span>Integrations</span>
                        </button>
                        <button
                          onClick={() => setHealthChecksModalOpen(true)}
                          className={`btn-swipe w-full flex items-center gap-3 p-2.5 ${theme === "dark" ? "bg-neutral-800/50 border-neutral-700/50" : "bg-gray-100 border-gray-200"} border rounded-lg text-sm ${textPrimary} action-hover-glow`}
                        >
                  ...
                  {widgetVisibility.recentActivity && (
                    <div className={`${bgCard} border rounded-xl p-4 card-hover-glow`}>
                      ...
                      <div className="space-y-3">
                        {activities.length === 0 && (
                          <p className={`text-sm ${textMuted}`}>No recent activity</p>
                        )}
                          ...
                          <div key={activity.id} className="flex gap-3">
                            ...
                            <div>
                              ...
                              <p className={`text-xs ${textMuted}`}>
                                {activity.domain_name ? `${activity.domain_name} • ` : ""}
              ...
              {widgetVisibility.analyticsCharts && (
                <div className="mt-8">
                  <div className={`${bgCard} border rounded-xl overflow-hidden card-hover-glow`}>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {chartTabs.map((tab) => (
                          <button
                            ...
                            className={`pill-hover-glow px-4 py-2 rounded-full text-sm font-medium ${activeChart === tab.id ? "bg-red-600 text-white" : `${theme === "dark" ? "bg-neutral-800 text-neutral-400" : "bg-gray-100 text-gray-500"}`}`}
                          >
                            {tab.label}
                          </button>
                        ))}
                    ...
                    <div className="p-4 sm:p-6 min-h-[280px]">{renderChart()}</div>
                  </div>
                </div>
              )}
            </>
          ...
          <div className="px-4 sm:px-6 lg:px-8 py-8">
            <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
              <div className={`flex items-center gap-4 ${textSecondary} text-sm`}>
                ...
                {loadTime && (
                  <span className={`text-xs ${textMuted}`}>Dashboard loaded in {loadTime.toFixed(1)}s</span>
                )}
              </div>
              ...
              <div className="flex items-center gap-4">
                <div className={`flex items-center gap-2 px-3 py-1.5 ${bgInput} border rounded-lg`}>
                  ...
                  <span className={`text-xs ${theme === "dark" ? "text-neutral-300" : "text-gray-600"}`}>
                    ICANN Accredited
                  </span>
                </div>
                <div className={`flex items-center gap-2 px-3 py-1.5 ${bgInput} border rounded-lg`}>
              ...
              <nav className="flex flex-wrap items-center justify-center gap-4 sm:gap-6">
                ...
                <button onClick={() => setStatusModalOpen(true)} className={`text-sm ${textSecondary} hover:text-red-400`}>
                  System Status
                </button>
        ...
        <nav className={`lg:hidden fixed bottom-0 left-0 right-0 ${bgSidebar} border-t z-30 safe-area-pb`}>
          <div className="flex items-center justify-around py-2 px-2">
            ...
            <button
              ...
              className={`flex flex-col items-center gap-1 p-3 min-w-[56px] min-h-[56px] justify-center ${textSecondary}`}
            >
        ...
        {selectedDomains.length > 0 && (
          ...
          >
            <div
              className={`flex items-center gap-3 px-4 py-3 ${theme === "dark" ? "bg-neutral-800 border-neutral-700" : "bg-white border-gray-300"} border rounded-xl shadow-2xl`}
            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className={`relative ${bgDropdown} border rounded-2xl shadow-2xl w-full max-w-md`} style={{ animation: "scaleIn 0.3s ease-out" }}>
            ...
            <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
              {(
                [
                  { key: "statsCards" as const, label: "Stats Cards", description: "Total Domains, SSL, Visits, Uptime" },
                  { key: "domainsTable" as const, label: "Your Domains Table", description: "Domain listing with actions" },
                  { key: "quickActions" as const, label: "Quick Actions", description: "Integrations, Health Checks, etc." },
                  { key: "recentActivity" as const, label: "Recent Activity", description: "Latest domain activities" },
                  { key: "analyticsCharts" as const, label: "Analytics Charts", description: "Domain growth, SSL status, traffic" },
                  { key: "pinnedDomains" as const, label: "Pinned Domains", description: "Your favorite domains" },
                  { key: "recentlyViewed" as const, label: "Recently Viewed", description: "Last viewed domains" },
                ] as const
              ).map((widget) => (
                <div
                  key={widget.key}
                  className={`flex items-center justify-between p-3 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-lg`}
                >
                  <div>
                  ...
                  <button
                    onClick={() => updateWidgetVisibility(widget.key, !widgetVisibility[widget.key])}
                    className={`p-2 rounded-lg transition-colors ${widgetVisibility[widget.key] ? "bg-red-500/10 text-red-400" : `${theme === "dark" ? "bg-neutral-700" : "bg-gray-200"} ${textMuted}`}`}
                  >
                    {widgetVisibility[widget.key] ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
                  </button>
                </div>
              ))}
              <button
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    {shortcut.keys.map((key, i) => (
                      <span key={i} className={`kbd ${theme === "dark" ? "text-neutral-300" : "text-gray-600"}`}>
                        {key}
                      </span>
                    ))}
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className={`relative ${bgDropdown} border rounded-2xl shadow-2xl w-full max-w-lg`} style={{ animation: "scaleIn 0.3s ease-out" }}>
            ...
            <div className="p-6 space-y-6">
              <div>
                <label className={`block text-sm font-medium ${theme === "dark" ? "text-neutral-300" : "text-gray-600"} mb-2`}>
                  Calculate for how many years?
                </label>
                <div className="flex items-center gap-4">
                  <input
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        <p className={`text-xs ${textMuted}`}>
                          ${domain.initialPrice} + ${domain.renewalPrice}/yr
                        </p>
                      </div>
                      <span className={`text-lg font-bold ${textPrimary}`}>${totalCost.toFixed(2)}</span>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className={`relative ${bgDropdown} border rounded-2xl shadow-2xl w-full max-w-md`} style={{ animation: "scaleIn 0.3s ease-out" }}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-3">
                {["Domain Registration", "DNS Services", "SSL Certificates", "Email Services", "API", "Dashboard"].map(
                  (service) => (
                    <div
                      key={service}
                      className={`flex items-center justify-between p-3 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-lg`}
      ...
      {domainSettingsOpen && selectedDomainForSettings && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div
            className={`relative ${bgDropdown} border rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col`}
          ...
          >
            ...
            <div className={`flex items-center justify-between p-6 border-b ${borderColor} flex-shrink-0`}>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={closeDomainSettings} className={`p-2 ${textSecondary} ${bgHover} rounded-lg`}>
                  <X className="h-5 w-5" />
                </button>
              </div>
            </div>
            ...
            <div className={`flex border-b ${borderColor} flex-shrink-0 overflow-x-auto`}>
              {(
                [
                  ...
                  { id: "dns" as const, label: "DNS Management", icon: Server },
                  { id: "email" as const, label: "Email & Forwarding", icon: Mail },
                ...
                <button
                  key={tab.id}
                  onClick={() => setDomainSettingsTab(tab.id)}
                ...
                <div className="space-y-6">
                  <div className={`p-4 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-xl`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className={`text-sm font-medium ${textPrimary}`}>Domain Lock</p>
                          <p className={`text-xs ${textSecondary}`}>Prevents unauthorized transfers</p>
                        </div>
                      </div>
                      <ToggleSwitch
                        enabled={domainSettingsForm.locked}
                        onChange={() => { setDomainSettingsForm((p) => ({ ...p, locked: !p.locked })); setHasUnsavedChanges(true) }}
                        theme={theme}
                      />
                    </div>
                  </div>
                  <div className={`p-4 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-xl`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className={`text-sm font-medium ${textPrimary}`}>Auto-Renew</p>
                          <p className={`text-xs ${textSecondary}`}>Next renewal: {selectedDomainForSettings.expiry}</p>
                        </div>
                      </div>
                      <ToggleSwitch
                        enabled={domainSettingsForm.autoRenew}
                        onChange={() => { setDomainSettingsForm((p) => ({ ...p, autoRenew: !p.autoRenew })); setHasUnsavedChanges(true) }}
                        theme={theme}
                      />
                    </div>
                  </div>
                  <div className={`p-4 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-xl`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className={`text-sm font-medium ${textPrimary}`}>WHOIS Privacy</p>
                          <p className={`text-xs ${textSecondary}`}>Hide personal info from public records</p>
                        </div>
                      </div>
                      <ToggleSwitch
                        enabled={domainSettingsForm.privacy}
                        onChange={() => { setDomainSettingsForm((p) => ({ ...p, privacy: !p.privacy })); setHasUnsavedChanges(true) }}
                        theme={theme}
                      />
                    </div>
                  </div>
                  <div className={`p-4 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-xl`}>
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        <p className={`text-sm font-medium ${textPrimary}`}>Registration Period</p>
                        <p className={`text-xs ${textSecondary}`}>Extend your registration</p>
                      </div>
                    </div>
                    <select
                      value={domainSettingsForm.registrationYears}
                      onChange={(e) => { setDomainSettingsForm((p) => ({ ...p, registrationYears: parseInt(e.target.value) })); setHasUnsavedChanges(true) }}
                      className={`w-full px-3 py-2 ${bgInput} border rounded-lg text-sm ${textPrimary}`}
                    >
                ...
                <div className="space-y-6">
                  <div className={`p-4 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-xl`}>
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div>
                        <p className={`text-sm font-medium ${textPrimary}`}>Nameservers</p>
                        <p className={`text-xs ${textSecondary}`}>Configure DNS management</p>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <label className="flex items-center gap-3 cursor-pointer">
                        <input
                          type="radio"
                          checked={!domainSettingsForm.useCustomNS}
                          onChange={() => { setDomainSettingsForm((p) => ({ ...p, useCustomNS: false })); setHasUnsavedChanges(true) }}
                          className="text-red-500"
                        />
                        <span className={`text-sm ${textPrimary}`}>Use DomainPro Nameservers (Recommended)</span>
                      </label>
                      <label className="flex items-center gap-3 cursor-pointer">
                        <input
                          type="radio"
                          checked={domainSettingsForm.useCustomNS}
                          onChange={() => { setDomainSettingsForm((p) => ({ ...p, useCustomNS: true })); setHasUnsavedChanges(true) }}
                          className="text-red-500"
                        />
                        <span className={`text-sm ${textPrimary}`}>Use Custom Nameservers</span>
                      </label>
                  ...
                  <div className={`p-4 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-xl`}>
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <DatabaseIcon className="h-5 w-5 text-red-400" />
                        <div>
                          <p className={`text-sm font-medium ${textPrimary}`}>DNS Records</p>
                          <p className={`text-xs ${textSecondary}`}>Manage A, AAAA, CNAME, MX, TXT records</p>
                        </div>
                      </div>
                      <button onClick={() => setAddDnsRecordOpen(true)} className="btn-swipe-red flex items-center gap-2 px-3 py-1.5 bg-red-600 text-white text-xs font-medium rounded-lg">
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className={`text-left border-b ${borderColor}`}>
                            <th className={`py-2 ${textSecondary} font-medium`}>Type</th>
                            <th className={`py-2 ${textSecondary} font-medium`}>Name</th>
                            <th className={`py-2 ${textSecondary} font-medium`}>Value</th>
                            <th className={`py-2 ${textSecondary} font-medium`}>TTL</th>
                            <th />
                          </tr>
                        </thead>
                        <tbody>
                          {dnsRecords.map((r) => (
                            <tr key={r.id} className={`border-b ${borderColor}`}>
                              <td className={`py-2 ${textPrimary}`}>
                                <span className="px-2 py-0.5 bg-red-500/10 text-red-400 rounded text-xs">{r.type}</span>
                              </td>
                              ...
                              <td className="py-2">
                                <button onClick={() => deleteDnsRecord(r.id)} className="p-1 text-red-400 hover:text-red-300">
                  ...
                  <div className={`p-4 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-xl`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className={`text-sm font-medium ${textPrimary}`}>DNSSEC</p>
                          <p className={`text-xs ${textSecondary}`}>Add cryptographic security to DNS</p>
                        </div>
                      </div>
                      <ToggleSwitch
                        enabled={domainSettingsForm.dnssec}
                        onChange={() => { setDomainSettingsForm((p) => ({ ...p, dnssec: !p.dnssec })); setHasUnsavedChanges(true) }}
                        theme={theme}
                      />
                    </div>
                  </div>
                  ...
                  <div className={`p-4 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-xl`}>
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className={`text-sm font-medium ${textPrimary}`}>Email Forwarding</p>
                          <p className={`text-xs ${textSecondary}`}>Forward emails to external addresses</p>
                        </div>
                      </div>
                      <button
                        onClick={() => setEmailForwarding((p) => [...p, { id: Date.now(), from: "", to: "", active: true }])}
                        className="btn-swipe-red flex items-center gap-2 px-3 py-1.5 bg-red-600 text-white text-xs font-medium rounded-lg"
                      >
                        <Plus className="h-3 w-3" />
                        Add Rule
                    ...
                    <div className="space-y-3">
                      {emailForwarding.map((rule, i) => (
                        <div
                          key={rule.id}
                          className={`flex items-center gap-3 p-3 ${theme === "dark" ? "bg-neutral-900/50" : "bg-white"} rounded-lg border ${borderColor}`}
                        >
                          <input
                            placeholder="from@"
                            value={rule.from}
                            onChange={(e) => {
                              const newRules = [...emailForwarding]
                              newRules[i] = { ...newRules[i], from: e.target.value }
                              setEmailForwarding(newRules)
                              setHasUnsavedChanges(true)
                            }}
                            className={`flex-1 px-3 py-1.5 ${bgInput} border rounded-lg text-sm ${textPrimary}`}
                          />
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              const newRules = [...emailForwarding]
                              newRules[i] = { ...newRules[i], to: e.target.value }
                              setEmailForwarding(newRules)
                              setHasUnsavedChanges(true)
                            }}
                            className={`flex-1 px-3 py-1.5 ${bgInput} border rounded-lg text-sm ${textPrimary}`}
                          />
                          <button
                            onClick={() => { setEmailForwarding(emailForwarding.filter((r) => r.id !== rule.id)); setHasUnsavedChanges(true) }}
                ...
                <div className="space-y-6">
                  <div className={`p-4 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-xl`}>
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className={`text-sm font-medium ${textPrimary}`}>SSL/TLS Certificate</p>
                          <p className={`text-xs ${textSecondary}`}>
                            {selectedDomainForSettings.ssl
                              ? `Expires: ${selectedDomainForSettings.sslExpiry}`
                              : "No SSL installed"}
                          </p>
                        </div>
                      </div>
                      <button
                        className={`btn-swipe px-3 py-1.5 ${selectedDomainForSettings.ssl ? "bg-emerald-600" : "bg-red-600"} text-white text-xs font-medium rounded-lg`}
                      >
                        {selectedDomainForSettings.ssl ? "Renew SSL" : "Install SSL"}
                      </button>
                    </div>
                    {selectedDomainForSettings.ssl && (
                      <div
                        className={`p-3 ${theme === "dark" ? "bg-emerald-500/10" : "bg-emerald-50"} border border-emerald-500/30 rounded-lg flex items-center gap-2`}
                      >
                        <CheckCircle className="h-4 w-4 text-emerald-400" />
                        <span className="text-sm text-emerald-400">SSL Active • Auto-renewal enabled</span>
                      </div>
                    )}
                  </div>
                  <div className={`p-4 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-xl`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className={`text-sm font-medium ${textPrimary}`}>2FA for Domain Changes</p>
                          <p className={`text-xs ${textSecondary}`}>Require 2FA for sensitive operations</p>
                        </div>
                      </div>
                      <ToggleSwitch
                        enabled={domainSettingsForm.twoFactorForChanges}
                        onChange={() => { setDomainSettingsForm((p) => ({ ...p, twoFactorForChanges: !p.twoFactorForChanges })); setHasUnsavedChanges(true) }}
                        theme={theme}
                      />
                    </div>
                  </div>
                  <div className={`p-4 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-xl`}>
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        <p className={`text-sm font-medium ${textPrimary}`}>EPP/Authorization Code</p>
                        <p className={`text-xs ${textSecondary}`}>Required for domain transfers</p>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <input
                        type={showEppCode ? "text" : "password"}
                        value="ABCD-1234-EFGH-5678"
                        readOnly
                        className={`flex-1 px-3 py-2 ${bgInput} border rounded-lg text-sm ${textPrimary}`}
                      />
                      <button onClick={() => setShowEppCode(!showEppCode)} className={`px-3 py-2 ${bgInput} border rounded-lg`}>
                        {showEppCode ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                      </button>
                      <button className={`px-3 py-2 ${bgInput} border rounded-lg`}>
                ...
                <div className="space-y-6">
                  <div className={`p-4 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-xl`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className={`text-sm font-medium ${textPrimary}`}>Transfer Domain Out</p>
                          <p className={`text-xs ${textSecondary}`}>Move to another registrar</p>
                        </div>
                      </div>
                      <button className="btn-swipe px-3 py-1.5 bg-yellow-600 text-white text-xs font-medium rounded-lg">
                        Initiate Transfer
                      </button>
                    </div>
                  </div>
                  <div className={`p-4 ${theme === "dark" ? "bg-neutral-800/50" : "bg-gray-100"} rounded-xl`}>
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        <p className={`text-sm font-medium ${textPrimary}`}>Webhook URL</p>
                        <p className={`text-xs ${textSecondary}`}>Receive domain event notifications</p>
                      </div>
                    </div>
                    <input
                      type="url"
                      placeholder="https://your-server.com/webhook"
                      value={domainSettingsForm.webhookUrl}
                      onChange={(e) => { setDomainSettingsForm((p) => ({ ...p, webhookUrl: e.target.value })); setHasUnsavedChanges(true) }}
                      className={`w-full px-3 py-2 ${bgInput} border rounded-lg text-sm ${textPrimary}`}
                    />
                  </div>
                  <div className={`p-4 bg-red-500/10 border border-red-500/30 rounded-xl`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className={`text-sm font-medium ${textPrimary}`}>Delete Domain</p>
                          <p className="text-xs text-red-400">Cancel registration and remove from account</p>
                        </div>
                      </div>
                      <button className="px-3 py-1.5 bg-red-600 text-white text-xs font-medium rounded-lg">
                        Delete Domain
                      </button>
                    </div>
                  </div>
            ...
            <div className={`flex items-center justify-between p-6 border-t ${borderColor} flex-shrink-0`}>
              <button onClick={closeDomainSettings} className={`px-4 py-2 ${bgInput} border rounded-lg text-sm ${textSecondary}`}>
                Cancel
              </button>
              <button
                onClick={saveDomainSettings}
                disabled={settingsSaving || !hasUnsavedChanges}
                className="btn-swipe-red flex items-center gap-2 px-4 py-2 bg-red-600 text-white text-sm font-medium rounded-lg disabled:opacity-50"
              >
                {settingsSaving ? (
                  <>
      ...
      {addDnsRecordOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div
            className={`relative ${bgDropdown} border rounded-2xl shadow-2xl w-full max-w-md`}
            style={{ animation: "scaleIn 0.3s ease-out" }}
          >
            <div className={`flex items-center justify-between p-6 border-b ${borderColor}`}>
              <h3 className={`text-lg font-extrabold ${textPrimary}`}>Add DNS Record</h3>
              <button onClick={() => setAddDnsRecordOpen(false)} className={`p-2 ${textSecondary} ${bgHover} rounded-lg`}>
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className={`text-sm font-medium ${textSecondary} block mb-1`}>Type</label>
                <select
                  value={newDnsRecord.type}
                  onChange={(e) => setNewDnsRecord((p) => ({ ...p, type: e.target.value }))}
                  className={`w-full px-3 py-2 ${bgInput} border rounded-lg text-sm ${textPrimary}`}
                >
              ...
              <div>
                <label className={`text-sm font-medium ${textSecondary} block mb-1`}>Name</label>
                <input
                  value={newDnsRecord.name}
                  onChange={(e) => setNewDnsRecord((p) => ({ ...p, name: e.target.value }))}
                  placeholder="@ or subdomain"
                  className={`w-full px-3 py-2 ${bgInput} border rounded-lg text-sm ${textPrimary}`}
                />
              </div>
              <div>
                <label className={`text-sm font-medium ${textSecondary} block mb-1`}>Value</label>
                <input
                  value={newDnsRecord.value}
                  onChange={(e) => setNewDnsRecord((p) => ({ ...p, value: e.target.value }))}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={`text-sm font-medium ${textSecondary} block mb-1`}>TTL</label>
                  <select
                    value={newDnsRecord.ttl}
                    onChange={(e) => setNewDnsRecord((p) => ({ ...p, ttl: parseInt(e.target.value) }))}
                    className={`w-full px-3 py-2 ${bgInput} border rounded-lg text-sm ${textPrimary}`}
                  >
                ...
                {newDnsRecord.type === "MX" && (
                  <div>
                    <label className={`text-sm font-medium ${textSecondary} block mb-1`}>Priority</label>
                    <input
                      type="number"
                      value={newDnsRecord.priority}
                      onChange={(e) => setNewDnsRecord((p) => ({ ...p, priority: parseInt(e.target.value) || 0 }))}
                      className={`w-full px-3 py-2 ${bgInput} border rounded-lg text-sm ${textPrimary}`}
                    />
                  </div>
                )}
            ...
            <div className={`flex justify-end gap-3 p-6 border-t ${borderColor}`}>
              <button onClick={() => setAddDnsRecordOpen(false)} className={`px-4 py-2 ${bgInput} border rounded-lg text-sm ${textSecondary}`}>
                Cancel
              </button>
              <button onClick={addDnsRecord} className="btn-swipe-red px-4 py-2 bg-red-600 text-white text-sm font-medium rounded-lg">
                Add Record
              </button>
            </div>
          </div>

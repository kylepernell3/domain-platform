...
function ThemeToggle({ theme, toggleTheme }: { theme: Theme; toggleTheme: () => void }) {
  return (
    <button
      onClick={toggleTheme}
      className={"p-2 rounded-lg transition-all " + (theme === "dark" ? "bg-white/10 hover:bg-white/20 text-yellow-400" : "bg-gray-100 hover:bg-gray-200 text-gray-700")}
...
function NavDropdown({ item, theme, isOpen, onToggle, onClose }: { item: NavItem; theme: Theme; isOpen: boolean; onToggle: () => void; onClose: () => void }) {
  ...
  if (!item.dropdown) {
    return (
      <Link href={item.href || "#"} className={"font-medium " + (theme === "dark" ? "text-gray-300 hover:text-white" : "text-gray-600 hover:text-gray-900")}>
        {item.label}
      </Link>
    )
  }
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={onToggle}
        className={"flex items-center gap-1 font-medium " + (theme === "dark" ? "text-gray-300 hover:text-white" : "text-gray-600 hover:text-gray-900")}
      >
        {item.label}
          ...
          <div className="p-2">
            {item.dropdown.map(di => (
              <Link
                key={di.href}
                href={di.href}
                className={"flex items-start gap-3 p-3 rounded-lg " + (theme === "dark" ? "hover:bg-white/10" : "hover:bg-gray-100")}
                onClick={onClose}
              >
    ...
    <div className={"fixed inset-0 z-50 " + (theme === "dark" ? "bg-gray-900" : "bg-white")}>
      ...
      <nav className="p-4 overflow-y-auto max-h-[calc(100vh-80px)]">
          ...
          <div key={item.label} className="mb-2">
            {item.dropdown ? (
              <>
                ...
                >
                  {item.label}
                  {expanded.has(item.label) ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                </button>
                  ...
                  <div className="ml-4 mt-1 space-y-1">
                    {item.dropdown.map(di => (
                      <Link
                        ...
                        onClick={onClose}
                      >
                        <span className="text-red-400">{di.icon}</span>
                        {di.label}
                      </Link>
                    ))}
                  </div>
        ...
        <div className="mt-6 pt-6 border-t border-gray-700">
          <Link href="/login" className="block w-full p-3 text-center font-bold text-white bg-red-500 hover:bg-red-600 rounded-xl" onClick={onClose}>
            Sign In
          </Link>
        </div>
      </nav>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/login" className={"hidden sm:flex px-4 py-2 rounded-lg font-bold " + (theme === "dark" ? "text-white hover:bg-white/10" : "text-gray-900 hover:bg-gray-100")}>
              Sign In
            </Link>
            <button className="lg:hidden p-2 rounded-lg hover:bg-white/10" onClick={() => setMobileOpen(true)}>
            ...
            <div key={section.title}>
              ...
              <ul className="space-y-3">
                {section.links.map(link => (
                  <li key={link.href}>
                    <Link href={link.href} className={"text-sm " + (theme === "dark" ? "text-gray-500 hover:text-white" : "text-gray-600 hover:text-gray-900")}>
                      {link.label}
                    </Link>
                  </li>
                ))}
        ...
        <div className={"pt-8 border-t " + (theme === "dark" ? "border-gray-800" : "border-gray-200")}>
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex gap-4">
              <a href="#" className={"p-2 rounded-lg " + (theme === "dark" ? "text-gray-500 hover:text-white" : "text-gray-400 hover:text-gray-600")}>
...
interface SearchBarProps {
  ...
  setShowBulkCheck: (show: boolean) => void
  bulkCheckList: BulkCheckItem[]
  setBulkCheckList: (list: BulkCheckItem[]) => void
  handleBulkCheck: (domains: string[]) => void
}
    ...
    <div className="space-y-4">
      ...
      <div className={"p-2 rounded-2xl " + (theme === "dark" ? "bg-gray-800/80 border border-gray-700" : "bg-white border border-gray-200 shadow-lg")}>
        <div className="flex items-center gap-2 relative">
          <Search className={"absolute left-4 h-5 w-5 " + (theme === "dark" ? "text-gray-500" : "text-gray-400")} />
          <input
            type="text"
            value={query}
            ...
            className={"w-full pl-12 pr-4 py-4 text-lg rounded-xl border-0 " + (theme === "dark" ? "bg-gray-900 text-white placeholder-gray-500" : "bg-gray-50 text-gray-900 placeholder-gray-400") + " focus:outline-none focus:ring-2 focus:ring-red-500"}
          />
          <button
            onClick={onSearch}
            disabled={isSearching}
            className="px-6 py-4 bg-red-500 hover:bg-red-600 disabled:bg-red-400 text-white font-bold rounded-xl flex items-center gap-2 transition-all"
          >
        ...
        {showSuggestions && suggestions.length > 0 && (
          <div className={"absolute left-0 right-0 mt-2 rounded-lg shadow-2xl border max-h-64 overflow-y-auto z-50 " + (theme === "dark" ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200")}>
            {suggestions.map((suggestion, idx) => (
              <button
                ...
                className={"w-full text-left px-4 py-3 transition-colors " + (theme === "dark" ? "hover:bg-gray-700" : "hover:bg-gray-100") + (idx === selectedSuggestionIndex ? (theme === "dark" ? " bg-gray-700" : " bg-gray-100") : "")}
              >
      ...
      <div className="flex flex-wrap items-center gap-4">
        <button
          ...
          className={"px-4 py-2 rounded-lg font-medium flex items-center gap-2 transition-all " + (theme === "dark" ? "bg-purple-600 hover:bg-purple-700 text-white" : "bg-purple-100 hover:bg-purple-200 text-purple-700")}
        >
        ...
        {searchHistory.length > 0 && (
          <button
            ...
            className={"px-4 py-2 rounded-lg font-medium flex items-center gap-2 transition-all " + (theme === "dark" ? "bg-gray-600 hover:bg-gray-700 text-white" : "bg-gray-200 hover:bg-gray-300 text-gray-700")}
          >
        ...
        <div className={"ml-auto text-sm " + (theme === "dark" ? "text-gray-400" : "text-gray-600")}>
          <kbd className={"px-2 py-1 rounded " + (theme === "dark" ? "bg-gray-700" : "bg-gray-200")}>Ctrl+K</kbd> or <kbd className={"px-2 py-1 rounded " + (theme === "dark" ? "bg-gray-700" : "bg-gray-200")}>/</kbd> to search
        </div>
      </div>

      {/* Bulk Check Panel */}
      {showBulkCheck && (
        <div className={"rounded-xl shadow-lg p-6 border " + (theme === "dark" ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200")}>
          <div className="flex items-center justify-between mb-4">
            <h3 className={"text-xl font-bold flex items-center gap-2 " + (theme === "dark" ? "text-white" : "text-gray-900")}>
              <Layers className="h-5 w-5 text-purple-600" />
              Bulk Domain Check
            </h3>
            <button onClick={() => setShowBulkCheck(false)} className={"hover:text-gray-700 " + (theme === "dark" ? "text-gray-500" : "text-gray-400")}>
              <X className="h-5 w-5" />
            </button>
          </div>
          <textarea
            className={"w-full h-32 p-3 border rounded-lg mb-3 " + (theme === "dark" ? "border-gray-600 bg-gray-900 text-white" : "border-gray-300 bg-white text-gray-900")}
          ...
          {bulkCheckList.length > 0 && bulkCheckList.some(item => item.status !== 'pending') && (
            <div className="mt-4 space-y-2 max-h-96 overflow-y-auto">
              {bulkCheckList.map((item, idx) => (
                <div key={idx} className={"flex items-center justify-between p-3 rounded-lg " + (theme === "dark" ? "bg-gray-900" : "bg-gray-50")}>
    ...
    <div className="flex flex-wrap justify-center gap-6 md:gap-12 mt-8">
      {TRUST_BADGES.map((badge, i) => (
        <div key={i} className="flex items-center gap-3">
          ...
          <div>
            <div className={"font-bold " + (theme === "dark" ? "text-white" : "text-gray-900")}>{badge.label}</div>
            <div className={"text-xs " + (theme === "dark" ? "text-gray-500" : "text-gray-400")}>{badge.sublabel}</div>
          </div>
        </div>
      ))}
    </div>
...
function DomainCardSkeleton({ theme, viewMode }: { theme: Theme; viewMode: ViewMode }) {
  if (viewMode === "list") {
    return (
      <div className={"p-4 rounded-xl border animate-pulse " + (theme === "dark" ? "bg-gray-800/50 border-gray-700" : "bg-white border-gray-200")}>
        <div className="flex items-center gap-4">
          <div className={"w-6 h-6 rounded-full " + (theme === "dark" ? "bg-gray-700" : "bg-gray-200")} />
          <div className={"h-6 w-48 rounded " + (theme === "dark" ? "bg-gray-700" : "bg-gray-200")} />
          <div className="flex-1" />
          <div className={"h-10 w-32 rounded-lg " + (theme === "dark" ? "bg-gray-700" : "bg-gray-200")} />
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={"p-6 rounded-2xl border animate-pulse " + (theme === "dark" ? "bg-gray-800/50 border-gray-700" : "bg-white border-gray-200")}>
      <div className={"h-6 w-3/4 rounded mb-4 " + (theme === "dark" ? "bg-gray-700" : "bg-gray-200")} />
      <div className={"h-4 w-1/2 rounded mb-4 " + (theme === "dark" ? "bg-gray-700" : "bg-gray-200")} />
      <div className={"h-12 w-full rounded-xl " + (theme === "dark" ? "bg-gray-700" : "bg-gray-200")} />
    </div>
  )
}
...
function DomainCard({
  ...
  const getStatusBadge = () => {
    ...
    if (result.available && result.premium) {
      return (
        <span className="px-2 py-1 text-xs font-medium rounded-full bg-yellow-500/20 text-yellow-400 flex items-center gap-1">
    ...
    return (
      <span className="px-2 py-1 text-xs font-medium rounded-full bg-red-500/20 text-red-400 flex items-center gap-1">
      ...
      <div className={"p-4 rounded-xl border transition-all " + (result.available ? (theme === "dark" ? "bg-gray-800/50 border-gray-700 hover:border-green-500/50" : "bg-white border-gray-200 hover:border-green-500") : (theme === "dark" ? "bg-gray-800/30 border-gray-700" : "bg-gray-50 border-gray-200"))}>
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className={"font-semibold truncate " + (theme === "dark" ? "text-white" : "text-gray-900")}>{result.domain}</div>
          </div>
          {getStatusBadge()}
          {result.available && result.price && (
            <div className={"font-bold " + (theme === "dark" ? "text-white" : "text-gray-900")}>
              {formatPrice(result.price)}
              <span className={"text-sm font-normal " + (theme === "dark" ? "text-gray-500" : "text-gray-400")}>/yr</span>
            </div>
          )}
          {result.available ? (
            <div className="flex items-center gap-2">
              <button
                ...
                className={"p-2 rounded-lg " + (inWatchlist ? "text-red-500" : theme === "dark" ? "text-gray-500 hover:text-white" : "text-gray-400 hover:text-gray-600")}
              >
              ...
              <button
                onClick={onAddToCart}
                disabled={inCart}
                className={"px-4 py-2 rounded-lg font-medium " + (inCart ? "bg-green-500 text-white" : "bg-red-500 hover:bg-red-600 text-white")}
              >
                {inCart ? <><Check className="h-4 w-4 inline mr-1" />Added</> : <><Plus className="h-4 w-4 inline mr-1" />Add</>}
              </button>
            </div>
          ) : (
            <button className={"px-4 py-2 rounded-lg font-medium " + (theme === "dark" ? "bg-gray-700 text-gray-400" : "bg-gray-200 text-gray-500")}>
              View Details
            </button>
          )}
        </div>
    ...
    <div className={"p-6 rounded-2xl border transition-all " + (result.available ? (theme === "dark" ? "bg-gray-800/50 border-gray-700 hover:border-green-500/50 hover:shadow-lg hover:shadow-green-500/10" : "bg-white border-gray-200 hover:border-green-500 hover:shadow-lg") : (theme === "dark" ? "bg-gray-800/30 border-gray-700" : "bg-gray-50 border-gray-200"))}>
      <div className="flex items-start justify-between mb-3">
        ...
        {result.available && (
          <button
            onClick={onToggleWatchlist}
            className={"p-1 rounded " + (inWatchlist ? "text-red-500" : theme === "dark" ? "text-gray-500 hover:text-white" : "text-gray-400 hover:text-gray-600")}
          >
            <Heart className={"h-5 w-5 " + (inWatchlist ? "fill-current" : "")} />
          </button>
        )}
      ...
      {result.available && result.price ? (
        <>
          <div className="mb-4">
            <span className={"text-2xl font-bold " + (theme === "dark" ? "text-white" : "text-gray-900")}>{formatPrice(result.price)}</span>
            <span className={"text-sm " + (theme === "dark" ? "text-gray-500" : "text-gray-400")}>/year</span>
            {result.renewalPrice && result.renewalPrice !== result.price && (
              <div className={"text-xs mt-1 " + (theme === "dark" ? "text-gray-500" : "text-gray-400")}>
                Renews at {formatPrice(result.renewalPrice)}/yr
              </div>
            )}
          </div>
          <button
            onClick={onAddToCart}
            disabled={inCart}
            className={"w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 " + (inCart ? "bg-green-500 text-white" : "bg-red-500 hover:bg-red-600 text-white")}
          >
            {inCart ? <><Check className="h-5 w-5" />In Cart</> : <><Plus className="h-5 w-5" />Add to Cart</>}
          </button>
        </>
        ...
        <div className={"text-sm " + (theme === "dark" ? "text-gray-500" : "text-gray-400")}>
          {result.expirationDate ? "Expires: " + new Date(result.expirationDate).toLocaleDateString() : "This domain is registered"}
        </div>
      )}
    </div>
  )
...
function FilterSidebar({
  ...
  const toggleTLD = (tld: string) => {
    setFilters({
      ...filters,
      tlds: filters.tlds.includes(tld) ? filters.tlds.filter(t => t !== tld) : [...filters.tlds, tld]
    })
  }
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          {POPULAR_TLDS.map(t => (
            <button
              ...
              className={"px-3 py-1.5 text-sm rounded-lg transition-all " + (filters.tlds.includes(t.tld) ? "bg-red-500 text-white" : theme === "dark" ? "bg-gray-700 text-gray-300 hover:bg-gray-600" : "bg-gray-100 text-gray-600 hover:bg-gray-200")}
            >
              .{t.tld}
            </button>
          ))}
      ...
      <div>
        ...
        <div className={"text-sm " + (theme === "dark" ? "text-gray-400" : "text-gray-500")}>
          Up to {formatPrice(filters.maxPrice)}/year
        </div>
      </div>
      <div className="space-y-3">
        ...
        <label className={"flex items-center gap-3 cursor-pointer " + (theme === "dark" ? "text-gray-300" : "text-gray-700")}>
          <input
            ...
            onChange={e => setFilters({ ...filters, excludeNumbers: e.target.checked })}
            className="w-4 h-4 accent-red-500"
          />
          Exclude numbers
        </label>
      ...
      <button
        ...
        className={"w-full py-2 rounded-lg text-sm " + (theme === "dark" ? "bg-gray-700 hover:bg-gray-600 text-gray-300" : "bg-gray-100 hover:bg-gray-200 text-gray-600")}
      >
        Reset Filters
      </button>
    </div>
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div className={"hidden lg:block w-64 flex-shrink-0 p-6 rounded-2xl border " + (theme === "dark" ? "bg-gray-800/50 border-gray-700" : "bg-white border-gray-200")}>
        {content}
      </div>
      ...
      {isOpen && (
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          <div className={"absolute bottom-0 left-0 right-0 max-h-[80vh] overflow-y-auto rounded-t-2xl p-6 " + (theme === "dark" ? "bg-gray-900" : "bg-white")}>
            <div className="flex items-center justify-between mb-6">
              <h2 className={"text-lg font-bold " + (theme === "dark" ? "text-white" : "text-gray-900")}>Filters</h2>
  ...
  return (
    <div className={"flex flex-wrap items-center justify-between gap-4 p-4 rounded-xl mb-4 " + (theme === "dark" ? "bg-gray-800/50" : "bg-gray-100")}>
      <div className={"font-medium " + (theme === "dark" ? "text-white" : "text-gray-900")}>
        {count} domain{count !== 1 ? "s" : ""} found
      </div>
      <div className="flex items-center gap-3">
        <button
          onClick={onOpenFilters}
          className={"lg:hidden flex items-center gap-2 px-3 py-2 rounded-lg " + (theme === "dark" ? "bg-gray-700 text-white" : "bg-white text-gray-700")}
        >
        ...
        <select
          ...
          className={"px-3 py-2 rounded-lg border " + (theme === "dark" ? "bg-gray-800 border-gray-700 text-white" : "bg-white border-gray-300")}
        >
        ...
        <div className={"flex rounded-lg overflow-hidden border " + (theme === "dark" ? "border-gray-700" : "border-gray-300")}>
          <button
            ...
            className={"p-2 " + (viewMode === "grid" ? "bg-red-500 text-white" : theme === "dark" ? "bg-gray-800 text-gray-400" : "bg-white text-gray-500")}
          >
            <Grid className="h-4 w-4" />
          </button>
          <button
            onClick={() => setViewMode("list")}
            className={"p-2 " + (viewMode === "list" ? "bg-red-500 text-white" : theme === "dark" ? "bg-gray-800 text-gray-400" : "bg-white text-gray-500")}
          >
    ...
    <div className="mb-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {promos.map(t => (
          <div
            key={t.tld}
            className={"p-4 rounded-xl border text-center transition-all hover:scale-105 " + (theme === "dark" ? "bg-gray-800/50 border-gray-700 hover:border-red-500/50" : "bg-white border-gray-200 hover:border-red-500")}
          >
            ...
            <div className="text-green-500 font-bold">
              {formatPrice(t.price)}
              <span className={"text-xs font-normal " + (theme === "dark" ? "text-gray-500" : "text-gray-400")}>/yr</span>
            </div>
          </div>
        ))}
      </div>
...
function DomainSearchContent() {
  ...
  const addToHistory = useCallback((searchQuery: string, count: number) => {
    ...
    setSearchHistory(prev => [newItem, ...prev.filter(h => h.query !== searchQuery)].slice(0, 10))
  }, [setSearchHistory])
  ...
  const addToWatchlist = useCallback((domain: string) => {
    ...
    if (existing) {
      console.log('Domain already in watchlist')
      return
    }
  ...
  const removeFromWatchlist = useCallback((domain: string) => {
    setWatchlist(prev => prev.filter(item => item !== domain))
    console.log(`Removed ${domain} from watchlist`)
  }, [setWatchlist])
  ...
  const checkWatchlist = useCallback(async () => {
    ...
    console.log('Checking watchlist domains...')
  ...
  const checkDomains = useCallback(async (domains: string[]) => {
    ...
    try {
      ...
      console.log('Batch check failed. Retrying with individual checks...')

      // Fallback to individual checks with retry logic
      for (let i = 0; i < domains.length; i++) {
        const data = await checkSingleDomainWithRetry(domains[i])
  ...
  }, [checkSingleDomainWithRetry, addToHistory])
  ...
  const filteredResults = useMemo(() => {
    ...
    if (filters.tlds.length > 0) {
      filtered = filtered.filter(r => filters.tlds.some(t => r.domain.endsWith("." + t)))
    }
    ...
    if (filters.excludeNumbers) {
      filtered = filtered.filter(r => !/\d/.test(extractName(r.domain)))
    }
    if (filters.excludeHyphens) {
      filtered = filtered.filter(r => !extractName(r.domain).includes("-"))
    }
    filtered = filtered.filter(r => {
      const name = extractName(r.domain)
      return name.length >= filters.minLength && name.length <= filters.maxLength
    })
    ...
    switch (sortBy) {
      ...
      case "price-desc":
        ...
        break
      case "length":
        filtered.sort((a, b) => extractName(a.domain).length - extractName(b.domain).length)
        break
      case "name":
  ...
  return (
    <div className={"min-h-screen " + (theme === "dark" ? "bg-gray-900" : "bg-gray-50")}>
      ...
      <section className={"py-16 px-4 " + (theme === "dark" ? "bg-gradient-to-b from-gray-800 to-gray-900" : "bg-gradient-to-b from-white to-gray-50")}>
        <div className="max-w-4xl mx-auto text-center">
          <h1 className={"text-4xl md:text-5xl font-bold mb-4 " + (theme === "dark" ? "text-white" : "text-gray-900")}>
            Find Your <span className="text-red-500">Perfect Domain</span>
          </h1>
          <p className={"text-xl mb-8 " + (theme === "dark" ? "text-gray-400" : "text-gray-600")}>
            Search millions of domains with real-time availability
          </p>
          <SearchBar
            theme={theme}
            query={query}
            ...
            showBulkCheck={showBulkCheck}
            setShowBulkCheck={setShowBulkCheck}
            bulkCheckList={bulkCheckList}
            setBulkCheckList={setBulkCheckList}
                  ...
                  <div className={"grid gap-4 " + (viewMode === "grid" ? "md:grid-cols-2 lg:grid-cols-3" : "")}>
                    {filteredResults.map(result => (
                      <DomainCard
                        ...
                        theme={theme}
                        viewMode={viewMode}
                        onAddToCart={() => result.price && addToCart(result.domain, result.price)}
                        inCart={cart.some(i => i.domain === result.domain)}
                        onToggleWatchlist={() => toggleWatchlist(result.domain)}
                        inWatchlist={watchlist.includes(result.domain)}
                      />
                    ))}
                  </div>
                ) : (
                  <div className={"p-12 rounded-2xl border text-center " + (theme === "dark" ? "bg-gray-800/50 border-gray-700" : "bg-white border-gray-200")}>
                    <Search className={"h-16 w-16 mx-auto mb-4 " + (theme === "dark" ? "text-gray-600" : "text-gray-300")} />
      ...
      {/* TLD Promo Section (when no search) */}
      {!hasSearched && (
